...
// .    .    .    .    .    .    .    .    .    .    .    .    .    .    .    .    .    .    .    .    .    .
	...
	'; }
		...
		elseif ($art == "Eingabefeld") {
			...
			else $form .= str_replace(array('#cont#', '#desc#', '#style#'), array($data, $desc, $style), $designStd);
		}
		
		elseif ($art == "Upload") {
			$data = '<input type="file" name="file" id="'.$feld.'" name="'.$desc.'"'.$pflicht. ' placeholder="'.$desc.$star.'" type="text" class="form-control" required />';
			$form .= str_replace(array('#cont#', '#desc#', '#style#'), array($data, $desc, $style), $designStd);
			$uploadForm = 1;
		}
...
*/
	...
	';
		...
		elseif ($art == "Mitteilungsfeld") {
			...
			$form .= str_replace(array('#cont#', '#desc#', '#style#', '#zeile2#'), array($data, $desc, $style, '<p class="hilfe">'.nl2br($hilfe).'</p>'), $designStd);
		}
