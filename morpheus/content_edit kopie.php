...
# # $b_chk enthaelt den namen des brick = name der php-vorlage aus dem ordner bricks
				...
				elseif (isin("secure",$b_chk))											$insert_secure = 1;
...
#				elseif (preg_match("/presse/", $b_chk)) 								$insert_presse = 1;
				...
				elseif (preg_match("/veranstaltung/", $b_chk)) 								$insert_termin = 1;
...
<table bgcolor=\"$bgcolor\" id=\"t".$counter."\" class=\"item\">
	<tr>";
		...
		<td class="c150" valign="top"> &nbsp; '.$counter.'.  <a href="content_fckedit.php?cid='.$edit.'&navid='.$navid.'&back='.$back.'&stelle='.$counter.'&db='.$db.'" class="textedit">' .str_replace("_", " ", $b_chk) .' &nbsp; <i class="fa fa-pencil-square-o large"></i></a>
					...
					$textbox .= "<td><textarea class=\"summernote\" name=\"brick#" .$brick[0] ."#" .$counter ."\" readonly style=\"width:510px; background-color:#f2f2f2; border: solid 4px $colour;\" onchange=\"setchange('1');\">". stripslashes($txt) ."</textarea>
				</td>
				...
				elseif ($insert_form || $insert_templ || $insert_termin || $insert_news || $insert_loesung || $galerie || $insert_branche || $insert_pdf || $insert_shop || $insert_anwendung || $insert_mindflash || $insert_kunde || $insert_mitarbeiter || $insert_objekt || $insert_presse || $insert_video || $insert_menu || $insert_vorlage || $insert_warengruppe || $insert_land || $insert_farbe || $insert_icon || $insert_colour || $insert_klasse || $insert_pdf_group || $insert_buchung || $insert_referenz || $insert_masterslider || $insert_material || $insert_oberflaeche || $insert_farbpalette) {
					...
					elseif ($insert_secure) {
						unset($insert_secure);
						$pdf_pd = pulldownContent ($txt, "morp_media", "pgname", "pgid");
					}
...
<ul class="produktmap">
		<li class="closeHeaderProdukt"><i class="fa fa-close"></i></li>
';

// + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + +
// + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + +
// INSERT LIST OF ALL PDF
$sql_pdf = "SELECT ugID, ugName FROM cc_artikeluntergruppe WHERE aktiv=1 ORDER BY ugName";
$res_pdf = safe_query($sql_pdf);
while ($row_pdf = mysqli_fetch_object($res_pdf)) {
	echo '<li><a data-clipboard-text="<a:produkt#'.$row_pdf->ugID.':>Link_Name</a>" class="intLink">'.$row_pdf->ugName.'</a></li>';
...
.pdfmap, .produktmap { padding: 10px 20px 20px 40px; }
.sitemap a, .sitemap li, .pdfmap a, .pdfmap li, .produktmap a, .produktmap li {
	color: #fff;
}
.closeHeader, .closeExtLinkHeader, .closemailadrHeader, .closeHeaderPDF, .closeHeaderProdukt { background: #222; list-style: none; height: 30px; text-align: right; border: solid 1px #1997c6; width: 50px; padding: 6px 10px 5px; display: block; float: right;
}
...
$( document ).ready(function() {
    ...
    $( ".closeHeaderProdukt" ).click(function() {
		$('.produktmap').hide(500);
	});
