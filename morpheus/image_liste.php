...
$english_alt = $_REQUEST["english_alt"];
...
elseif ($newtext || $ltext || ($filename != $filenameORG)) {
	if($filename != $filenameORG) {
		if(rename("../images/userfiles/image/".$filenameORG, "../images/userfiles/image/".$filename)) {
			$query = "UPDATE `morp_cms_image` SET itext='".addslashes($newtext)."', `longtext`='".addslashes($ltext)."', `english_alt`='".addslashes($english_alt)."', imgname='$filename' WHERE imgid=$inr";
			safe_query($query);
		}
	}
	else {
		$query = "UPDATE `morp_cms_image` SET itext='".addslashes($newtext)."', `longtext`='".addslashes($ltext)."', `english_alt`='".addslashes($english_alt)."' WHERE imgid=$inr";
		safe_query($query);
	}
	create_img_liste();
}
