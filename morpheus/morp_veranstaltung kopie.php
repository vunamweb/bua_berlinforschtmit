...
if ($_REQUEST["search_event"] == '' && $_REQUEST['get_log'] == '') {
	//echo 'ok'; die();
...
-->			
			...
			<td align="right">' . $row->eventAnzahlTeilnehmer . ' &nbsp; &nbsp; </td>
			<td align="right">' . $eventAnzahlRest . ' &nbsp; &nbsp; </td>
			<td>
				' . $si . '
			</td>
			<td>
...
function getLogEvent($typeEvent) {
    ...
    while ($row = mysqli_fetch_object($res)) {
        ...
        $log .= '<a href="#" rel="tooltip" data-html="true" title="'.$title.'" class="text_log">' . $row->mail_text . '</a>';
...
function edit($edit)
{
    global $arr_form, $hide, $table, $tid, $imgFolder, $um_wen_gehts, $scriptname, $mylink, $table2, $tid2, $ilink;
    ...
    $echo .= '
        ...
        <input type="Hidden" name="value_type_event" id="value_type_event">
        <input type="Hidden" name="value_text_mail" id="value_text_mail">
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button class="btn send_mail_event_user" data-dismiss="modal">Save</button>
        </div>
...
if ($save) {
    ...
    if($back == 2) {
        ...
        $sql = 'insert into morp_event_user_mail(idEvent, mail_text, type_event, date_create) values('.$eventId.', "'.$userMail.'", '.$typeEvent.', "'.$date.'")';
        //echo $sql;
        safe_query($sql);
        ...
        while ($row = mysqli_fetch_object($result)) {
            ...
            sendMailSMTP($to, utf8_decode($subject), utf8_decode($mail_txt));
        }

        saveMorpheusForm($edit, $neu); */
