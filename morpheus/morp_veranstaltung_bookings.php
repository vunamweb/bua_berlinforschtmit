...
echo '<div id=vorschau>
	...
	<form action="" onsubmit="" name="verwaltung" id="form_veranstaltung_bookings" method="post">
'.($edit || $neu ? '' : '').'
'.(!$edit && !$neu ? '' : '').'
';
...
$arr_form = array(
		...
		array("confirm", "Confirm", 'chk'),
		array("register", "Register", 'chk'),
		array("receive_mail", "Cancel", 'chk'),
...
function edit($edit) {
	global $arr_form, $table, $tid, $imgFolder, $um_wen_gehts, $scriptname, $mylink;
	global $table2, $tid2, $nameField2, $table3, $tid3, $nameField3, $table4, $tid4, $nameField4;

	echo $sql = "SELECT * FROM $table t1, $table4 t2 WHERE t1.event=t2.$tid4 AND $tid=".$edit."";
	$res = safe_query($sql);
	$row = mysqli_fetch_object($res);

	$register = $row->register;

	$receive_mail_text = ($row->receive_mail) ? 'Yes' : 'No';
	$receive_mail_value = ($row->receive_mail) ? 1 : 0;

	$check_receive_mail = ($row->receive_mail) ? '<input type="checkbox" id="check_receive_mail">&nbsp;&nbsp;<label>Send mail to user</label><br></br>' : '';
// print_r($row);
...
// $nameField4 = "eventName";
	...
	foreach($arr_form as $arr) {
		$checkbox = setMorpheusForm($row, $arr, $imgFolderShort, $edit, $scriptname, $tid);
	}

	$echo .= '<table class="autocol">
    <tr><td>eventName </td><td> &nbsp; </td><td> <b>'.$row->eventName.' am '.euro_dat($row->datum).'</b>	</td></tr>
	<tr><td>Received Mail</td><td> &nbsp; </td><td> '.$receive_mail_text.'</td></tr>
    ...
    <tr><td>paymentid </td><td> &nbsp; </td><td> '.$row->paymentid.'	</td></tr>
			...
			<br>
				<button type="submit" id="savebtn" class="btn btn-success hide"><i class="fa fa-save"></i> &nbsp; '.$um_wen_gehts.' speichern / aktualisieren</button>
				<button type="submit" id="savebtn2" value="hier" class="btn btn-success hide"><i class="fa fa-save"></i> &nbsp; '.$um_wen_gehts.' speichern und zurück</button>
			<br>
			<br>
			<!-- '.$checkbox.' !-->
			<br>
				<a href="?storno='.$edit.'" class="btn btn-info hide"><i class="fa fa-close"></i> &nbsp; STORNO / CANCEL </a>
				<a class="btn btn-success confirm_user_event">Confirm</a>
...
<script>
	  ...
	  $('.confirm_user_event').click(function(e){
        ...
        // if ready send mail
		if($('#check_receive_mail').length > 0 && !$('#check_receive_mail').is(':checked')) {
			$('#savebtn2').click();
		} else {
			//e.preventDefault();
            $('#receive_mail').prop('checked', true);
			...
			$.ajax({
				...
				type: "post",
				data: "id="+id+"&confirm="+confirm+"&register="+register+"",	
