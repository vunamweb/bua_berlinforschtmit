...
';
	...
	while ($row = mysqli_fetch_object($res)) {
		...
		$sql = "SELECT $nameField2 FROM `$table2` WHERE $tid2=$getDataID";
		$rs = safe_query($sql);
...
<table class="autocol p20 newTable" style="width:100%">
	<tr>
		<td>Name</td>
		<td align="center">Anzahl Plätze</td>
		<td>Adresse</td>
		<td>Phone</td>
		<td>E-Mail</td>
		<td>Date</td>
	...
	while($row = mysqli_fetch_object($res)) {
		...
		if($row->$mail) $all_mails .= $row->$mail.',';
		$summe += $row->quantity;
		$echo .= '
	<tr>
		<td>'.$row->name.'</td>
	...
	<br><br>
	<h3>Alle E-Mail Adressen mit Komma Trennung:</h3>'.substr($all_mails,0,strlen($all_mails)-1);
