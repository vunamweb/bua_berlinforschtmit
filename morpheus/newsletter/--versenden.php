...
$dir = 'http://www.apothekerkammer.de/';
...
if($live && !$livetrue) {
	...
	if (mysql_num_rows($ress) < 1) { echo '<br><br>KEINE NEWSLETTER MEHR ZU VERSENDEN !!!!'; die(); }
		...
		while($row = mysql_fetch_object($ress)) {
			...
			$sql = "SELECT f1name, f1link FROM morp_newsletter_filter_1 WHERE f1vi=1 ORDER by f1sort";
			$rs = safe_query($sql);
			while($rw = mysql_fetch_object($rs)) {
				$link = $rw->f1link;
				$lak_nav .= '<a href="'.($link ? $link : '#'.$rw->f1name).'" style="font-family: Arial, Helvetica, sans-serif;font-size: 14px;line-height:20px;color: #000;text-decoration: none;font-weight: bold;">'.$rw->f1name.'</a><br>';
			}
		...
		'.$extratext;
			...
			$raus[] = "/#weblink#/";
			$rein[] = 'http://www.apothekerkammer.de/service/lak+aktuell/lak+aktuell+ausgabe-'.$nlid.'/';
...
#			print_r($rein);
			...
			$data 	= preg_replace(array("/<!-- vid -->/","/&lt;!--%20vid%20--&gt;/","/<!-- mail -->/","/&lt;!--%20mail%20--&gt;/","/<img src=\"\/nldownloads/","/#spacer#/"), array($row->vid, $row->vid, $row->email, $row->email, '<img src="http://www.apothekerkammer.de/nldownloads', '<img src="http://www.apothekerkammer.de/timthumb.php?src=img/spacer.png&space='.$row->vid.'"/>'), $data);
