	...
	foreach ($arr_H as $val) {
		...
		if ($lastid != $val) {
			...
			if ( $ebene < 2 ) {
				...
				$lnk = '<a'.($accesskey != '' ? ' accesskey="'.$accesskey.'"' : '').' href="'.$url.'" class="'.($image_link ? 'd-none d-lg-block ' : '').'nav-link DPD" itemprop="name">'.($image_link ? '<img src="'.$xmlpfad.'images/navbar/'.$image_link.'" alt="'.$nm.'" class="img-fluid" width="120" />' : $nm).'SPAN</a>';
...
';
			...
			elseif ( $ebene == 4 ) {
				...
				$footer_set .= (!$sub_on3 ? '
							<ul class="dropdown-menu" itemprop="url">' : '');

				// parameter werden gesetzt. subnav vorhanden !!!!
				$sub_on3 = 1;

				$footer_set .= '
								<li><a'.($accesskey != '' ? ' accesskey="'.$accesskey.'"' : '').' href="'.$xmlpfad.($multilang ? $lan.'/' : '').$navID[$val].'" class="s'.($nid).'" itemprop="name">'.($nm).'</a></li>';
